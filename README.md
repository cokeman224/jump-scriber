...
3.  **Configure Environment Variables:**
    ...
    * Edit the `.env` file (or set environment variables directly) with your actual credentials:
        ...
        * `SALESFORCE_CLIENT_ID`: Your Salesforce Connected App Consumer Key (optional).
        * `SALESFORCE_CLIENT_SECRET`: Your Salesforce Connected App Consumer Secret (optional).
        * `SALESFORCE_REDIRECT_URI`: `"http://localhost:4000/auth/salesforce/callback"` (optional).
        * `SALESFORCE_BASE_URL`: Optional. Defaults to `"https://test.salesforce.com"` (sandbox). Set to `"https://login.salesforce.com"` for production.
...
* Salesforce modal + OAuth: `mix test test/social_scribe_web/live/salesforce_modal_test.exs test/ueberauth/strategy/salesforce/oauth_test.exs`
* HubSpot modal: `mix test test/social_scribe_web/live/hubspot_modal_test.exs`
...
## Salesforce Integration & Ask Anything Chat

* **Salesforce OAuth:** Connect a Salesforce org via Settings (Connected App with scopes `api`, `refresh_token`, `offline_access`). Defaults to sandbox (`test.salesforce.com`). Callback URL: `http://localhost:4000/auth/salesforce/callback`. Token refresh runs via Oban (SalesforceTokenRefresher).
* **Update in Salesforce:** From a meeting detail page, if Salesforce is connected, an "Update Salesforce Contact" button opens a modal to search contacts, view AI-suggested field updates from the transcript, and sync selected fields to the Contact record.
* **Ask Anything:** The "Ask Anything" chat (sidebar) works with both HubSpot and Salesforce. Tag a contact ("@ Add context") from either CRM; answers use that contact and relevant meeting transcripts, with sources shown below each response.
