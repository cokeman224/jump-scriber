defmodule SocialScribeWeb.ModalComponents do
          ...
          >
            <span class="flex items-center">
              ...
              <span class="ml-1.5 block truncate text-slate-900">
                <%= if (name = String.trim("#{@selected_contact.firstname || ""} #{@selected_contact.lastname || ""}")) != "" do %>
                  {name}
                <% else %>
                  {@selected_contact.email || "Contact"}
                <% end %>
              </span>
            </span>
          ...
          >
            ...
            <div>
              <div class="text-sm font-medium text-slate-900">
                <%= if (name = String.trim("#{contact.firstname || ""} #{contact.lastname || ""}")) != "" do %>
                  {name}
                <% else %>
  ...
  @doc """
  Renders a Salesforce-styled modal wrapper (same structure as hubspot_modal, Salesforce branding).
  """
  attr :id, :string, required: true
  attr :show, :boolean, default: false
  attr :on_cancel, JS, default: %JS{}
  ...
  def salesforce_modal(assigns) do
    ...
    <div
      ...
      class="relative z-50 hidden"
    >
      <div id={"#{@id}-bg"} class="bg-slate-500/90 fixed inset-0 transition-opacity" aria-hidden="true" />
      <div
        class="fixed inset-0 overflow-y-auto"
        aria-labelledby={"#{@id}-title"}
        aria-describedby={"#{@id}-description"}
