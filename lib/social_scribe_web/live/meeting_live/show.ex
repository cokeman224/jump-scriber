defmodule SocialScribeWeb.MeetingLive.Show do
  ...
  def handle_info({:hubspot_search, query, credential}, socket) do
    case HubspotApi.search_contacts(credential, query) do
      ...
      {:error, reason} ->
        send_update(SocialScribeWeb.MeetingLive.HubspotModalComponent,
          id: "hubspot-modal",
          error: "Failed to search contacts: #{format_crm_error(reason)}",
          searching: false
        )
    end
  ...
  def handle_info({:generate_suggestions, contact, meeting, _credential}, socket) do
    case HubspotSuggestions.generate_suggestions_from_meeting(meeting) do
      ...
      {:error, reason} ->
        send_update(SocialScribeWeb.MeetingLive.HubspotModalComponent,
          id: "hubspot-modal",
          error: "Failed to generate suggestions: #{format_crm_error(reason)}",
          loading: false
        )
    end
  ...
  def handle_info({:apply_hubspot_updates, updates, contact, credential}, socket) do
    case HubspotApi.update_contact(credential, contact.id, updates) do
      ...
      {:error, reason} ->
        send_update(SocialScribeWeb.MeetingLive.HubspotModalComponent,
          id: "hubspot-modal",
          error: "Failed to update contact: #{format_crm_error(reason)}",
          loading: false
        )
  ...
  defp normalize_contact(contact) do
    # Contact is already formatted with atom keys from HubspotApi.format_contact / SalesforceApi
    contact
  end

  defp format_crm_error(reason) do
    case reason do
      {:api_error, status, %{"message" => msg}} when is_binary(msg) ->
        "API error (#{status}): #{msg}"

      {:api_error, status, %{"error" => msg}} when is_binary(msg) ->
        "API error (#{status}): #{msg}"

      {:api_error, status, body} when is_map(body) ->
        msg = body["message"] || body["error"] || body["error_description"] || inspect(body)
        "API error (#{status}): #{msg}"
      ...
      {:token_refresh_failed, _} ->
        "Session expired. Please reconnect your account in Settings."
