defmodule SocialScribeWeb.AuthController do
  ...
  def callback(%{assigns: %{ueberauth_auth: auth, current_user: user}} = conn, %{
    ...
    raw_token = auth.extra.raw_info.token
    instance_url = raw_token.other_params["instance_url"]
  ...
  def callback(%{assigns: %{ueberauth_failure: failure}} = conn, _params) do
    ...
    |> redirect(to: redirect_after_failure(conn))
  end
  ...
  defp redirect_after_failure(conn) do
    if conn.assigns[:current_user], do: ~p"/dashboard/settings", else: ~p"/"
  end
end
